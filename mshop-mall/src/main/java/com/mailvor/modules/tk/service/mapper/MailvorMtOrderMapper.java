...
public interface MailvorMtOrderMapper extends CoreMapper<MailvorMtOrder> {
    ...
    @Override
    @Select("SELECT * FROM mailvor_mt_order ${ew.customSqlSegment}")
    <E extends IPage<MailvorMtOrder>> E selectPage(E page, @Param(Constants.WRAPPER) Wrapper<MailvorMtOrder> queryWrapper);

    @Update("update mailvor_mt_order set uid = ${uid} where unique_item_id = #{id}")
    ...
    @Select("select trade_parent_id from mailvor_mt_order where uid=#{uid} and order_pay_time <= #{time} and cash=0")
    Set<MailvorMtOrder> selectCashOrderId(@Param("uid") Long uid, @Param("time") Date time);

    @Update("update mailvor_mt_order set cash = 1 where uid=#{uid} and order_pay_time <= #{time} and cash=0")
    void updateCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select( "select IFNULL(sum(actual_item_amount),0)  from mailvor_mt_order " +
            "where is_del=0")
    Double sumTotalPrice();

    @Select( "select IFNULL(sum(balance_amount),0)  from mailvor_mt_order " +
            "where is_del=0")
    Double sumTotalFee();

    @Select("SELECT IFNULL(sum(actual_item_amount),0) " +
            " FROM mailvor_mt_order ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(sum(actual_item_amount),0) as num," +
            ...
            " FROM mailvor_mt_order where is_del=0 and inner_type=0 and order_pay_time >= #{time}" +
...
//    @Update("update mailvor_mt_order set tk_status = 13" +
//            " where trade_parent_id in" +
//            " <foreach collection='ids' index='index' item='id' separator=',' open='(' close=')'>" +
//            " #{id}" +
//            " </foreach>")
//    void invalidRefundOrders(@Param("ids") List<Long> ids);
    @Update("update mailvor_mt_order set item_status=3,bind=3,hb=0.0" +
