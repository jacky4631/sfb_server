...
//                .app("抖音")
    ...
    private LambdaQueryWrapper<MailvorEleOrder> getHbWrapper(Date start, Date end, boolean isVip) {
        LambdaQueryWrapper<MailvorEleOrder> wrapper = new LambdaQueryWrapper<>();
        if(isVip) {
            wrapper.gt(MailvorEleOrder::getShopHb, 0);
        } else {
            wrapper.eq(MailvorEleOrder::getShopHb, 0);
        }
