...
public interface MailvorTbOrderMapper extends CoreMapper<MailvorTbOrder> {
    ...
    @Override
    @Select("SELECT * FROM mw_tb_order ${ew.customSqlSegment}")
    <E extends IPage<MailvorTbOrder>> E selectPage(E page, @Param(Constants.WRAPPER) Wrapper<MailvorTbOrder> queryWrapper);

    @Update("update mw_tb_order set uid = ${uid} where trade_parent_id = #{id}")
    void bindUser(@Param("uid")Long uid, @Param("id") Long id);

    @Update("update mw_tb_order set bind = 2 where trade_parent_id = #{id}")
    void refundOrder(@Param("id") Long id);

    @Select("select IFNULL(sum(integral),0) from mw_tb_order " +
            "where uid=#{uid} and tk_create_time <= #{time} and bind=1 and cash=0")
    double totalCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select("select trade_parent_id from mw_tb_order where uid=#{uid} and tk_create_time <= #{time} and cash=0")
    Set<MailvorTbOrder> selectCashOrderId(@Param("uid") Long uid, @Param("time") Date time);

    @Update("update mw_tb_order set cash = 1 where uid=#{uid} and tk_create_time <= #{time} and cash=0")
    void updateCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select( "select IFNULL(sum(alipay_total_price),0)  from mw_tb_order " +
            "where is_del=0")
    Double sumTotalPrice();

    @Select( "select IFNULL(sum(pub_share_pre_fee),0)  from mw_tb_order " +
            "where is_del=0")
    Double sumTotalFee();

    @Select("SELECT IFNULL(sum(alipay_total_price),0) " +
            " FROM mw_tb_order ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(sum(alipay_total_price),0) as num," +
            ...
            " FROM mw_tb_order where is_del=0 and inner_type=0 and tk_create_time >= #{time}" +
...
//    @Update("update mw_tb_order set tk_status = 13" +
//            " where trade_parent_id in" +
//            " <foreach collection='ids' index='index' item='id' separator=',' open='(' close=')'>" +
//            " #{id}" +
//            " </foreach>")
//    void invalidRefundOrders(@Param("ids") List<Long> ids);
    @Update("update mw_tb_order set tk_status = 13,bind=3,hb=0.0" +
