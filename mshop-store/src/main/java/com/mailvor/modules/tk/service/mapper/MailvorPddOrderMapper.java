...
public interface MailvorPddOrderMapper extends CoreMapper<MailvorPddOrder> {
    ...
    @Override
    @Select("SELECT * FROM mw_pdd_order ${ew.customSqlSegment}")
    <E extends IPage<MailvorPddOrder>> E selectPage(E page, @Param(Constants.WRAPPER) Wrapper<MailvorPddOrder> queryWrapper);

    @Update("update mw_pdd_order set uid = ${uid} where order_sn = #{id}")
    void bindUser(@Param("uid")Long uid, @Param("id") String orderSn);
    @Update("update mw_pdd_order set bind = 2 where order_sn = #{id}")
    void refundOrder(@Param("id") String orderSn);


    @Select("select IFNULL(sum(integral),0) from mw_pdd_order " +
            "where uid=#{uid} and order_create_time <= #{time} and bind=1 and cash=0")
    double totalCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select("select order_sn from mw_pdd_order where uid=#{uid} and order_create_time <= #{time} and cash=0")
    Set<MailvorPddOrder> selectCashOrderId(@Param("uid") Long uid, @Param("time") Date time);


    @Update("update mw_pdd_order set cash = 1 where uid=#{uid} and order_create_time <= #{time} and cash=0")
    void updateCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select( "select IFNULL(sum(order_amount),0)  from mw_pdd_order " +
            "where is_del=0")
    Double sumTotalPrice();

    @Select( "select IFNULL(sum(promotion_amount),0)  from mw_pdd_order " +
            "where is_del=0")
    Double sumTotalFee();

    @Select("SELECT IFNULL(sum(order_amount),0) " +
            " FROM mw_pdd_order ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(sum(order_amount),0) as num," +
            ...
            " FROM mw_pdd_order where is_del=0 and inner_type=0 and order_create_time >= #{time}" +
...
//    @Update("update mw_pdd_order set order_status = 4" +
//            " where order_sn in" +
...
//    void invalidRefundOrders(@Param("ids") String[] ids);
    @Update("update mw_pdd_order set order_status = 4,bind=3,hb=0.0" +
            " where order_sn = #{id}")
