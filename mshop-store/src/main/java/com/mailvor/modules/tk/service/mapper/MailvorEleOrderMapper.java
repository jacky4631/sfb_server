...
public interface MailvorEleOrderMapper extends CoreMapper<MailvorEleOrder> {
    ...
    @Override
    @Select("SELECT * FROM mw_ele_order ${ew.customSqlSegment}")
    <E extends IPage<MailvorEleOrder>> E selectPage(E page, @Param(Constants.WRAPPER) Wrapper<MailvorEleOrder> queryWrapper);

    @Update("update mw_ele_order set uid = ${uid} where trade_id = #{id}")
    void bindUser(@Param("uid")Long uid, @Param("id") String orderId);
    @Update("update mw_ele_order set bind = 2 where trade_id = #{id}")
    void unbindUser(@Param("id") String orderId);


    @Select("select IFNULL(sum(integral),0) from mw_ele_order " +
            "where uid=#{uid} and pay_time <= #{time} and bind=1 and cash=0")
    double totalCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select("select trade_id from mw_ele_order where uid=#{uid} and pay_time <= #{time} and cash=0")
    Set<MailvorEleOrder> selectCashOrderId(@Param("uid") Long uid, @Param("time") Date time);


    @Update("update mw_ele_order set cash = 1 where uid=#{uid} and pay_time <= #{time} and cash=0")
    void updateCash(@Param("uid") Long uid, @Param("time") Date time);

    @Select( "select IFNULL(sum(pay_price),0)  from mw_ele_order " +
            "where is_del=0")
    Double sumTotalPrice();

    @Select( "select IFNULL(sum(predict_money),0)  from mw_ele_order " +
            "where is_del=0")
    Double sumTotalFee();

    @Select("SELECT IFNULL(sum(pay_price),0) " +
            " FROM mw_ele_order ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(sum(pay_price),0) as num," +
            ...
            " FROM mw_ele_order where is_del=0 and inner_type=0 and pay_time >= #{time}" +
...
//    @Update("update mw_ele_order set flow_point = 'REFUND'" +
//            " where trade_id in" +
...
//    void invalidRefundOrders(@Param("ids") List<String> ids);
    @Update("update mw_ele_order set order_status = 3,bind=3,hb=0.0" +
            " where trade_id = #{id}")
